...
const templateWithJQuery = () =>{
  starWarsPeople.forEach(element => {
    let getHtml = $('#template').html();
    const $newSection = $(`<section>${getHtml}</section>`)
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(ingredient =>{
    ...
    result.push(ingredient.slice(ingredient.indexOf(' ', 4)+1,ingredient.leng))
  });
  return result;
};
