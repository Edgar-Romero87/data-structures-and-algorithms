...
const findHappiness = (arr) => arr.filter(string => string.includes(':)') ? string : false);
...
const standardizePhoneNumbers = (arr) => arr.map(element => `${element.substring(1, 4)}${element.substring(6, 9)}${element.substring(10, 15)}`);
...
const onlyOddChars = (str) => [...str].reduce((element, current, i) => i %2 !== 0 ? element += current : element,'');

// return [...str].reduce((element, current, i) => {
//   if(i %2 !== 0){
